    ...
    if conf.has_section(section):
        for option in essential_options:
            if conf.has_option(section, option):
                ...
                aopm.error(f"The config file dont have the key: '{option}' in '{section}' :(", True)
    else:
        aopm.error(f"The config file dont have the section '{section}' :(", True)
...
if os.geteuid() != 0:
    aopm.error("To run AOPM you must be root!", True)

# check the arguments
argc = len(sys.argv)
if argc < 2:
    aopm.error("Too few arguments :(\nTry: aopm help me", True)
...
try:
    ...
    aopm.error(f"No Module named: '{module_to_search}' found :(", True)

if hasattr(module, "get_header"):
    module_header = module.get_header()
else:
    aopm.error(f"The module: '{module_to_search}' dont have the 'get_header' function :(", True)

module_reader = aopm.Api_header(module_header)
if module_reader.check():
    if hasattr(module, "run"):
        ...
        aopm.error(f"The module: '{module_to_search}' dont have the 'run' function :(", True)
else:
    aopm.error(f"The module: '{module_to_search}' dont have a valid header :(", True)